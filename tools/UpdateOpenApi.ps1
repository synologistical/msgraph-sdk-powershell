...
$ModuleMapping.Keys | ForEach-Object {
    ...
    if (-not ($v1Excludes -contains $ModuleName -and $GraphVersion -eq "v1.0")) {
        try {
            ...
            # 1. Temp download the OpenAPI file from metadata repo.
            # 2. Install Hidi on build agent.
            # 3. Run Hidi to slice the OpenAPI file and save in modules.
            # 4. Delete the temp OpenAPI file.
